...
export const GET_ALL_RELATIONS_TYPES_QUERY =
  "MATCH ()-[r]->() RETURN COLLECT(DISTINCT type(r)) AS relations";
export const SEARCH_QUERY = (s: string | null) =>
  `MATCH (n) WHERE toLower(n.name) CONTAINS toLower('${s}') RETURN n;`;
...
export const DELETE_QUERY = (name: string | null) =>
  `MATCH (g:GOD { name: '${name}' }) DELETE g`;
export const IMMEDIATE_RELATION_QUERY = (
  ...
  const query = relations
    ?.map(
      (relation) =>
        `MATCH (n {name: '${name}'})-[${getRelation(
          relation
        )}]->(related_node) RETURN related_node`
    )
    .join(" UNION ");
...
export const VERBOSE_SHORTEST_PATH_QUERY = (
  name1: string | null,
  name2: string | null,
  relations: string[] | undefined
) => {
...
export const nodeQueryBuilder = (params: NodeParams, limit: string | null) => {
  ...
  return FIND_NODES_QUERY.replace("[WHERE]", w).replace("[LIMIT]", l);
};
